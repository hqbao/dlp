...
export function validate() {
	var errors = gUIApp.nodeManager.checkConnectableNodeParams();
	if (errors.length > 0) {
		gExtEventMap.onError(errors);
		return;
	}

	errors = gUIApp.nodeManager.checkConnections();
	if (errors.length > 0) {
		gExtEventMap.onError(errors);
		return;
	}

	var errors = gUIApp.nodeManager.checkShapes();
	gExtEventMap.onError(errors);
}
